...
class DefaultController extends Controller
{
    ...
    public function testAction(Request $request){
     ...
     public function telecontact_dataAction(Request $request, $code_firme){
        ...
        if(!empty($ses_id_user)){
            ...
            INNER JOIN erpprod.u_yf_ssingleorders_inventory i ON i.id = s.ssingleordersid
            INNER JOIN erpprod.vtiger_service se ON se.serviceid = i.name
            WHERE s.support = 'telecontact' 
                AND s.type_ordre = 'Internet' 
                AND code_produit IN ('VA', 'VB', 'V0', 'V1') 
                AND acc.code_firme = $code_firme
            GROUP BY s.num_ordre,acc.code_firme, acc.raison_sociale, s.date_ordre, code_produit;");
     ...
     public function audienceAction(Request $request){
        ...
        if(!empty($ses_id_user)){
            ...
            //$clientId = '83342803379-2unaellkrn158ob6cr4gkslgbke1qqtv@developer.gserviceaccount.com';
            //$privateKeyFile =__DIR__.'/../../../../app/Resources/client_secrets.p12';
            ////$profileId = 'ga:10523831';
            //$profileId = "ga:21206490";
            $clientId = 'telecontactapi@telecontactma.iam.gserviceaccount.com';
            $privateKeyFile ="";
            $profileId = 'ga:353828619';
            $httpAdapter = new CurlHttpAdapter();
            $client = new Client($clientId, $privateKeyFile, $httpAdapter);
            $token = $client->getAccessToken();
                    //            $profileId = 'ga:10523831';
           ...
           // $query->setFilters(array("ga:eventLabel=@".$code_firme));
            ...
            $query_device->setFilters(array("ga:eventLabel=@".$code_firme));
            ...
            $query_users->setFilters(array("ga:eventLabel=@".$code_firme));
            ...
            $query_new_users->setFilters(array("ga:eventLabel=@".$code_firme));
            ...
            $query_pages->setFilters(array("ga:eventLabel=@".$code_firme));
     ...
     public function clickVisiteAction(Request $request){
        ...
        if(!empty($ses_id_user)){
            ...
            $response = new JsonResponse();
            return $response->setData(array('apparition_rs' => $apparition_rs,'fiche_apparition' => $fiche_apparition, 'begin_period' => $begin_period, 'end_period' => $end_period, 'testFirm' => $firmTesting));
        }else{
            return $this->redirect($this->generateUrl('connection'));
        }
    ...
    public function rubriqueAction(Request $request){
        ...
        if(!empty($ses_id_user)){
            ...
            $hostname2='100.1.1.8:3306';
            $dbname2='BD_EDICOM';
            $username='presencemedia';
            $password='tX632tpv39jD5KRC';
            $connection2 = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$hostname2;dbname=$dbname2",$username,$password)));
    ...
    if(!empty($ses_id_user)){
        ...
        $hostname2='100.1.1.8:3306';
        $dbname2='BD_EDICOM';
        $username2='presencemedia';
        $password2='tX632tpv39jD5KRC';
    ...
    public function clickAction($code_firme, Request $request){
        ...
        $session->set("codeFirme", $code_firme);
    ...
    public function clickNewAction($code_firme, Request $request){
        ...
        if(!empty($ses_id_user)){
            ...
            return $this->render('EcommerceBundle:Administration:statistics/clickNew.html.twig', array('data_vignette' => $data_vignette/*, 'data_user' => $data_user*/));
    ...
    public function clickTableAction(Request $request){
        ...
        if(!empty($ses_id_user)){
            ...
            $code_firme = $session->get("codeFirme");
            $start_period = $request->get('start_period');
            $end_period = $request->get('end_period');
     ...
     public function clickTableNewAction(Request $request) {
        $session = $request->getSession();
        $ses = $session->get('utilisateur');
        $ses_id_user = $ses[0]['id_user'];
        ...
        if (!empty($ses_id_user)) {
            ...
            if (!empty($sp)) {
                ...
                $start_period = '-1months';
                $end_period = '-1days';
                $selectedDaterange = array('start_period' => $start_period, 'end_period' => $end_period);
                $session->set('selectedDaterange', $selectedDaterange);
            }
            $jsonFile =__DIR__ . "/dev-telecontact-d659e99d9f7f.json";
                ...
                //     // Par exemple, afficher le contenu du tableau
                //     var_dump($credentials);
                // } else {
                //     // Le fichier JSON n'existe pas
                //     echo "Le fichier JSON n'existe pas.";
                // }
            $credentials = CredentialsLoader::fromFile($jsonFile);
            $client = new BetaAnalyticsDataClient();
            $client->setCredentials($credentials);
            
            $propertyId = '353828619'; 
            ...
            $dimension->setName('eventName');
            
            $metric = new Metric();
            $metric->setName('eventCount');
            
            $filter1 = new Filter();
            $filter1->setFieldName('eventName');

            $inListFilter = new InListFilter();
            $inListFilter->setValues(array('element_apparition', 'element_click', 'element_click_tel', 'Site_Web', 'element_click_message', 'element_click_rdv', 'element_click_devis'));
            $filter2->setStringFilter($stringFilter);
            
            $filterExpressionList = new FilterExpressionList();
            $filterExpressionList->setExpressions(array($filter1, $filter2));
            
            $filterExpression = new FilterExpression();
            $filterExpression->setAndGroup($filterExpressionList);
            
            // Utilisation des objets créés dans la requête
    ...
    public function uploadPdfImagesAction(Request $request){
        ...
        die();
            ...
            //             $hostname1='100.1.1.8:3306';
            //             $dbname1='espace_clients';
            //             $username1='presencemedia';
            //             $password1='tX632tpv39jD5KRC';
            //             $connection1 = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$hostname1;dbname=$dbname1",$username1,$password1)));
            //             $MAcode_firme = 'MA'.$code_firme;
            //             $raison_sociale = $connection1->fetchAll("
            //                 SELECT f.`rs_comp` as raison_sociale
    ...
    public function pdfAction(Request $request, $code_firme, $id_user, $start, $end) {
        ...
        if (!empty($ses_id_user)) {
            ...
            $pubsDirectory = '/var/www/prod/telecontactV2/telecontact/pubs/logos/';
            $imageFileName = 'L' . $code_firme . '.png';
            $imagePath = $pubsDirectory . $imageFileName;
    
            if (file_exists($imagePath)) {
                $url = $pubsDirectory . $imageFileName;
            } else {
                $url = '/var/www/prod/telecontactV2/telecontact/pubs/logos/L2107435.png';
            }
            ...
            $pdfPath = '/var/www/prod/espace-client/web/public_statistics/espace_stats/pdf_uploads/'.$id_user.'__'.$code_firme.'__'.date('Y-m-d').'.pdf';
            $html2pdf->Output($pdfPath, 'F');
            $html2pdf->Output($pdfPath, 'I');
    ...
    public function listpdfAction(Request $request, $code_firme){
     ...
     public function ticketListAction(Request $request){
        ...
        if(!empty($ses_id_user)){
            ...
            $connection = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$hostname;dbname=$dbname;charset=utf8",$username,$password)));
            $connection3= $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$hostname2;dbname=$dbname3;charset=utf8",$username,$password)));
            ...
            //           WHERE ti.code_firme = CONVERT(res.code_firme USING utf8)
            //     ');
            ...
            $Allticket = $connection3->fetchAll('SELECT *
            FROM espace_clients.`ticket` ti
            INNER JOIN (
                SELECT acc.code_firme, acc.raison_sociale, acc.email1, emp.last_name
                FROM erpprod.`u_yf_ssingleorders` s
                ...
                INNER JOIN erpprod.vtiger_account acc ON acc.accountid = s.firme
                WHERE s.support = "Telecontact" AND s.type_ordre = "Internet"
                GROUP BY acc.code_firme, acc.raison_sociale, acc.email1, emp.last_name
                ) res ON ti.code_firme = CONVERT(res.code_firme USING utf8);
            ');
            ...
            //       WHERE ti.code_firme = CONVERT(res.code_firme USING utf8)
            //        ");
            $ticket = $connection3->fetchAll("SELECT * FROM espace_clients.ticket ti 
            INNER JOIN (
                SELECT acc.code_firme, acc.raison_sociale, acc.email1, emp.last_name
                FROM erpprod.`u_yf_ssingleorders` s
                INNER JOIN erpprod.vtiger_ossemployees emp ON emp.ossemployeesid = s.comm
     ...
     public function avis_listAction(Request $request, $code_firme){
        ...
        if(!empty($ses_id_user)){
            ...
            LEFT JOIN espace_clients.utilisateurs ecu ON ecu.id_user = r.client_id
            WHERE r.code_firme = "'.$code_firme.'"
            GROUP BY ecu.raison_sociale, r.id, r.titre_commentaire, r.commentaire, r.date_creation, r.note_globale, r.publication, r.etat, re.id, re.comment_id, re.response, re.date_creation
            ');
...
if (!empty($prestations_arr)){
    ...
    $curl_arr = curl_init();
    curl_setopt($curl_arr, CURLOPT_URL, 'http://edicomelastic1.odiso.net:9200/telecontact42/_doc/MA'.$code_firme.'?pretty=true');
    curl_setopt($curl_arr,CURLOPT_HTTPHEADER, $header_arr);
    curl_setopt ($curl_arr, CURLOPT_CAINFO, "cacert.pem");
    curl_setopt($curl_arr, CURLOPT_RETURNTRANSFER, 1);
    $res_arr = curl_exec($curl_arr);
    curl_close($curl_arr);
    $response_arr =json_decode($res_arr, true);

      $total_arr        = $response_arr['_source']['rs_comp'];
      $poid        = $response_arr['_source']['poid'];
      ...
      if ($total_arr ){
        ...
        foreach ($prestations_arr as &$value) {

  $prest_arr[] = array(
                            'mot' => $this->replace_microsolft_apostrophe(($value)),
                            'code' => '',
                            'slug' => $this->slugify($this->adilsoft_string_latin($value)),
                        );
      ...
      // print_r($prestations_arr);  

               $mot_cle1_arr =   $response_arr["_source"]['mot_cle1'];

               if (!empty($prest_arr)){
                                 $mot_cle1_arr       =array_merge($mot_cle1_arr, $prest_arr);
...
*/
                               $doc = array(
                                 ...
                                 'actes_actu'        => $actes_arr,                                                         
                                 'langue_actu' => $langues_arr,
                                 'mot_cle1' => $mot_cle1_arr,
                                 'poid'     => $poid,
                                 'prests_actu_slugiy' =>  $prest_arr                                                    
        ...
        if(!empty($ses_id_user)){
            ...
            if ($request->isXmlHttpRequest() && $request->isMethod("POST") && $request->request->get("isDataTable") == "true") {
                ...
                if ($stmt->execute($params)) {
                    ...
                    // zahra
                        if ($stmt_count->execute(array($ses_id_user))) {
