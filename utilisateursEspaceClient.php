...
class   UtilisateursEspaceClientController extends Controller
{
    ...
    public function __construct()
    {
        ...
        array_push($this->dbName, "CRM_EDICOM");
        array_push($this->dbName, "espace_clients");
        array_push($this->dbName, "erpprod");

        array_push($this->hostName, "walibix2.odiso.net");
        array_push($this->username, "presencemedia");
        array_push($this->password, "tX632tpv39jD5KRC");
    ...
    public function connectionAction(Request $request) {
         if(isset($_SESSION['utilisateur']) && isset($_SESSION['data_user'])) {
         ...
         if(!empty($cf)) {
            ...
            if($request->isMethod('post')) {
                ...
                $host = $this->hostName[0];
                $db1 = $this->dbName[0];
                $db2 = $this->dbName[1];
                $db3 = $this->dbName[2];

                $connection = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$host;dbname=$db2",$this->username[0],$this->password[0])));
                $connection2 = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$host;dbname=$db1",$this->username[0],$this->password[0])));
                $connection3 = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$host;dbname=$db3",$this->username[0],$this->password[0])));
    ...
    public function reintialiserMdpAction(Request $request){
        ...
        if($request->isMethod('post')){
            $connectionFactory = $this->get('doctrine.dbal.connection_factory');
            $hostname='walibix2.odiso.net';
            $dbname='espace_clients';
            $connection = $connectionFactory->createConnection(array('pdo'=>new \PDO("mysql:host=$hostname;dbname=$dbname",$this->username[0],$this->password[0])));
    ...
    public function homepage2Action(Request $request, $code_firme){
        ...
        if(!empty($ses_id_user)){
            ...
            $dbname3='erpprod';
            $connection =$connectionFactory->createConnection(array('pdo'=>new \PDO("mysql:host=$hostname;dbname=$dbname",$this->username[0],$this->password[0])));
            $connection3 = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$hostname;dbname=$dbname3",$this->username[0],$this->password[0])));
            $queryClient ="
    ...
    public function homepageAction(Request $request, $code_firme="", $token=""){
        ...
        if(!empty($token)){
            ...
            $data_user = $session->get('utilisateur');
            $userData = null;
            if(isset($data_user[0]["code_firme"])){
                $userData = $data_user[0]["code_firme"];
            }
//
//            echo "=======";
//
//            print_r($data_user);
            ...
            return $this->render('EcommerceBundle:Administration:statistics/homepage2.html.twig', array('clients_telecontact'=>$clients_telecontact, 'client' =>$client, 'userData' => $userData));
    }
    ...
    public function addClientCommAction(Request $request, $code_commercial, $token=""){
        $code_commecial = 254;
        // TODO : Add customer
        if($request->request->get("isAdded") && $request->isMethod("POST")){
            $connectionFactory = $this->get('doctrine.dbal.connection_factory');
            $hostname='walibix2.odiso.net';
            $dbname3='erpprod';
            $connection = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$hostname;dbname=$dbname3",$this->username[0],$this->password[0])));

            $code_firme = $request->request->get("code_firme_input");
            $raison_social = $request->request->get("rs_input");
            $email = $request->request->get("email_input");

            $insertCustomer = $connection->prepare("INSERT INTO erpprod.vtiger_account(code_firme, raison_sociale, email1) VALUES(?,?,?)");
            if($insertCustomer->execute(array($code_firme , $raison_social , $email))){
                return new JsonResponse("Customer Added successfully !!");
            }
            else {
                return new JsonResponse("Une erreur c'est produite lors de l'ajout");
            }
        }
    ...
    public function ResCommercialAction(Request $request, $id_utilisateur, $token=""){
        ...
        if(!empty($ses_id_user)){
            ...
            $hostname='walibix2.odiso.net';
            $dbname2='CRM_EDICOM';
            $connection2 = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$hostname;dbname=$dbname2",$this->username[0],$this->password[0])));
                $findResCommercial = $connection2->fetchAll("
    ...
    public function disconnectAction(Request $request)
    {
        ...
        if(!empty($ses_id_user)){
             ...
             $connection = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$host;dbname=$db",$this->username[0],$this->password[0])));
    ...
    public function monCompteAction(Request $request, $code_firme)
    {
        ...
        if(!empty($ses_id_user)) {
            $connectionFactory = $this->get('doctrine.dbal.connection_factory');
                $dbname='espace_clients';
                $connection = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=walibix2.odiso.net;dbname=$dbname",$this->username[0],$this->password[0])));
    ...
    public function historique_connectionAction(Request $request){
        ...
        if(!empty($ses_id_user)){
            ...
            $connection        = $connectionFactory->createConnection(array('pdo' => new \PDO("mysql:host=$hostname;dbname=$dbname;charset=utf8",$this->username[0],$this->password[0])));
